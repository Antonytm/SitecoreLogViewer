...
bool SortFilesByCreationTime(FileEntry* i, FileEntry* j) { return  i> j; }

bool lastFileByTemplate(const char *normalisedDirectoryName, const char *templ, std::vector<FileEntry *> *&files) {
  if (!isDirectory(normalisedDirectoryName)) {
    return false;
  }
  files = new std::vector<FileEntry *>();
#if defined(OS_WIN)
  char *filenameSlashStar = new char[strlen(normalisedDirectoryName) + 2 + strlen(templ)];
  sprintf(filenameSlashStar, "%s\\%s", normalisedDirectoryName, templ);

  WIN32_FIND_DATAA file;
  WIN32_FIND_DATAA lastFile;
  ...
  while (filesRemaining) {
    if (FindNextFileA(handle, &file)) {
		if(lastFile.ftCreationTime.dwHighDateTime < file.ftCreationTime.dwHighDateTime)
	  {
		  lastFile = file;
	  }
      //pushFile(files, file);
...
bool listFiles(const char *normalisedDirectoryName, std::vector<FileEntry *> *&files) {
  ...
  files = new std::vector<FileEntry *>();
#if defined(OS_WIN)
  char *filenameSlashStar = new char[strlen(normalisedDirectoryName) + 4];
